...
\begin{enumerate}
	...
	\item 引用参考文献时，引用数字显示在正文中：\citen{2017_qinxiaohui_FeiYunZhiCheLiangDuiLieDeFenBuShiKongZhi}这样（\lstinline!\citen{}!）。
\end{enumerate}
...
如果插入图片的图注需要添加参考文献，如\figref{fig:ch2-1}的图注所示，为了防止前面的插图索引列表生成引用符号，同时防止参考文献顺序发生改变，只需在\lstinline!\caption!后面添加\lstinline![]!并在其中补充相应的题注信息，如\lstinline!\caption[湖南大学校徽]{湖南大学校徽~\cite{2021_chen_Graph}}!所示。
...
\section{公式书写与引用}
公式书写的开始形式如下
引用格式为\lstinline!\eqref{eq:ch1-1}!
\begin{equation}
  \label{eq:ch1-1}
  \mathbf{e}_{ij} = \pi\left(\mathbf{T}_{ij}\mathbf{p}_j\right) - \mathbf{p}_i
\end{equation}


\section{伪代码书写}
\begin{algorithm} [htb]           
    \DontPrintSemicolon %防止有些命令产生英文分号
	\caption{FCFS算法}
    \label{algo:revi-1}  
    \SetKwInOut{KwIn}{初始化}
    \SetKwInOut{KwOut}{输\quad 出}
	\KwIn{交叉路口信息，即网格占用点}
	\KwOut{$t_j^{\mathrm{in}}=\max \left\{t_{j, 1}^{\mathrm{in}}, t_{j, 2}^{\mathrm{in}}, \ldots, t_{j, k}^{\mathrm{in}}\right\}$ \quad $\rhd$}
